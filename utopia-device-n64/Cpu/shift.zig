...
pub fn fixed(comptime op: ShiftOp, core: *Core, word: u32) void {
    ...
    fw.log.trace("{X:08}: {t} {t}, {t}, {d}", .{ core.pc, op, args.rd, args.rt, args.sa });
    core.set(args.rd, op.apply(core.get(args.rt), args.sa));
}

pub fn fixed32(comptime op: ShiftOp, core: *Core, word: u32) void {
    const args: Core.RType = @bitCast(word);
    fw.log.trace("{X:08}: {t}32 {t}, {t}, {d}", .{ core.pc, op, args.rd, args.rt, args.sa });
    core.set(args.rd, op.apply(core.get(args.rt), @as(u6, args.sa) + 32));
}

pub fn variable(comptime op: ShiftOp, core: *Core, word: u32) void {
    const args: Core.RType = @bitCast(word);
    fw.log.trace("{X:08}: {t}V {t}, {t}, {t}", .{ core.pc, op, args.rd, args.rt, args.rs });
