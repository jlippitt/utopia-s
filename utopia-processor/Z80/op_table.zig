...
pub fn main(comptime iface: Core.Interface) [256]*const Core.Instruction {
    const bind = bindFn(iface);

    var ops: [256]*const Core.Instruction = undefined;
...
pub fn cb(comptime iface: Core.Interface) [256]*const Core.Instruction {
    const bind = bindFn(iface);

    var ops: [256]*const Core.Instruction = undefined;

    inline for (0..256) |opcode| {
        ops[opcode] = bind(invalid, .{ "CB", opcode });
    }
...
pub fn ed(comptime iface: Core.Interface) [256]*const Core.Instruction {
    const bind = bindFn(iface);

    var ops: [256]*const Core.Instruction = undefined;

    inline for (0..256) |opcode| {
        ops[opcode] = bind(invalid, .{ "ED", opcode });
    }

    // 0x40+6
    ops[0x46] = bind(implied.im, .{0});
    ops[0x4e] = bind(implied.im, .{0});
    ops[0x56] = bind(implied.im, .{1});
    ops[0x5e] = bind(implied.im, .{2});
    ops[0x66] = bind(implied.im, .{0});
...
fn bindFn(comptime iface: Core.Interface) BindFn {
    return struct {
        fn bind(comptime func: anytype, comptime args: anytype) Core.Instruction {
...
fn invalid(
    ...
    _ = core;
    _ = iface;
    fw.log.todo("Z80 opcode: {s}{X:02}", .{ prefix, opcode });
}
